...
public class MqConfiguration {
    ...
    public MqConfiguration(Document document) {
        MqSecuritySetting.internalRoleToLdapRoles = createLdapRoleMappings(document);
    ...
    MultiValuedMap<String, String> createLdapRoleMappings(Document document) {
        ...
        MultiValuedMap<String, String> map = new ArrayListValuedHashMap<>();
         ...
         document.getElementsByTag("role-mapping").stream().forEach(rolemapXml ->
            map.put(rolemapXml.attr("to"), rolemapXml.attr("from")));
         return map;
    }

    public List<MqEndpoint> getEndpoints() {
        return endpoints.stream().sorted(comparing(MqEndpoint::getName))
